...
impl ReplsetClient {
    ...
    pub(crate) fn send_command(&self, command: Command) -> Result<CommandResult> {
        ...
        let result = self.send_pending_command(&mut pending);
        self.remove_pending_command(pending.request_id);
        result
    }

    fn send_pending_command(&self, pending: &mut PendingCommand) -> Result<CommandResult> {
        let mut last_error = None;
        loop {
            let start = self
                .active_index
            ...
            for offset in 0..self.clients.len() {
                ...
                match client.send_command(pending.command.clone()) {

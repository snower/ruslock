...
fn init_ping_lock_commands_encode_to_64_byte_headers() {
    let init = Command::Init(InitCommand::new(Id16::from_bytes([1; 16]), Id16::from_bytes([2; 16])));
...
fn lock_data_encodes_java_frame_and_pipeline_lengths() {
    ...
    assert_eq!(set.encode().unwrap(), vec![5, 0, 0, 0, LOCK_DATA_COMMAND_TYPE_SET, 0, b'a', b'a', b'a']);
...
fn lock_result_data_parses_scalar_list_and_map_values() {
    let string_data = LockResultData::new(vec![5, 0, 0, 0, LOCK_DATA_COMMAND_TYPE_SET, 0, b'a', b'b', b'c']);
    assert_eq!(string_data.as_bytes(), Some(&b"abc"[..]));
    assert_eq!(string_data.as_string().unwrap(), "abc");

    let long_data = LockResultData::new({
        let mut raw = vec![10, 0, 0, 0, LOCK_DATA_COMMAND_TYPE_INCR, LOCK_DATA_FLAG_VALUE_TYPE_NUMBER];
    ...
    let list_data = LockResultData::new({
        let mut raw = vec![0, 0, 0, 0, LOCK_DATA_COMMAND_TYPE_PUSH, LOCK_DATA_FLAG_VALUE_TYPE_ARRAY];
    ...
    let map_data = LockResultData::new({
        let mut raw = vec![0, 0, 0, 0, LOCK_DATA_COMMAND_TYPE_SET, LOCK_DATA_FLAG_VALUE_TYPE_KV];
